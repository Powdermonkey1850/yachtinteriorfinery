...
class EL_GravityFormsStylerForDiviHelper extends ET_Builder_Module {

	/**
	 * Print Custom Background CSS
	 *
	 * @since 1.9.6
	 */
	public static function process_background( $args ) {
		...
		if ( isset( $args['backgrounds'], $args['module'] ) && ! empty( $args['backgrounds'] ) && ! empty( $args['module'] ) ) {
			if ( ET_BUILDER_PRODUCT_VERSION < '4.16.0' ) {
				self::process_old_background( $args );
			} else {
				foreach( $args['backgrounds'] as $basename => $css_element ) {
		            $bg_args = array(
		                'base_prop_name'                => $basename,
		                'props'                         => $args['props'],
		                'important'                     => ' !important',
		                'fields_definition'             => $args['fields'],
	...
	public static function process_multiple_checkboxes_value( $value, $values = array() ) {
	...
	public static function process_old_background( $args ) {
		...
		foreach ( $args['backgrounds'] as $basename => $css_element ) {
			...
			foreach ( et_pb_responsive_options()->get_modes() as $device ) {
				...
				if ( ! $is_desktop && ! et_pb_responsive_options()->is_responsive_enabled( $args['props'], "{$basename}_color" ) ) {
					continue;
				}

				// A. Background Gradient.
				$use_background_color_gradient = et_pb_responsive_options()->get_inheritance_background_value( $args['props'], "{$background_prefix}use_color_gradient", $device, $background_base_name, $args['fields'] );

				if ( 'on' === $use_background_color_gradient ) {
					$background_color_gradient_overlays_image = et_pb_responsive_options()->get_any_value( $args['props'], "{$background_prefix}color_gradient_overlays_image{$suffix}", '', true );

					$gradient_properties = array(
						'type'             => et_pb_responsive_options()->get_any_value( $args['props'], "{$background_prefix}color_gradient_type{$suffix}", '', true ),
						'direction'        => et_pb_responsive_options()->get_any_value( $args['props'], "{$background_prefix}color_gradient_direction{$suffix}", '', true ),
						'radial_direction' => et_pb_responsive_options()->get_any_value( $args['props'], "{$background_prefix}color_gradient_direction_radial{$suffix}", '', true ),
						'color_start'      => et_pb_responsive_options()->get_any_value( $args['props'], "{$background_prefix}color_gradient_start{$suffix}", '', true ),
						'color_end'        => et_pb_responsive_options()->get_any_value( $args['props'], "{$background_prefix}color_gradient_end{$suffix}", '', true ),
						'start_position'   => et_pb_responsive_options()->get_any_value( $args['props'], "{$background_prefix}color_gradient_start_position{$suffix}", '', true ),
						'end_position'     => et_pb_responsive_options()->get_any_value( $args['props'], "{$background_prefix}color_gradient_end_position{$suffix}", '', true ),
					);

					// Save background gradient into background images list.
					$background_images[] = $args['module']->get_gradient( $gradient_properties );
				...
				// B. Background Image.
				$background_image = et_pb_responsive_options()->get_inheritance_background_value( $args['props'], "{$background_prefix}image", $device, $background_base_name, $args['fields'] );
				$parallax         = et_pb_responsive_options()->get_any_value( $args['props'], "{$background_prefix}parallax{$suffix}", 'off' );
				...
				if ( $is_background_image_active ) {
					...
					// Size.
					$background_size_default = ET_Builder_Element::$_->array_get( $args['fields'], "{$background_prefix}size.default", '' );
					$background_size         = et_pb_responsive_options()->get_any_value( $args['props'], "{$background_prefix}size{$suffix}", $background_size_default, ! $is_prev_background_image_active );
					...
					// Position.
					$background_position_default = ET_Builder_Element::$_->array_get( $args['fields'], "{$background_prefix}position.default", '' );
					$background_position         = et_pb_responsive_options()->get_any_value( $args['props'], "{$background_prefix}position{$suffix}", $background_position_default, ! $is_prev_background_image_active );
					...
					// Repeat.
					$background_repeat_default = ET_Builder_Element::$_->array_get( $args['fields'], "{$background_prefix}repeat.default", '' );
					$background_repeat         = et_pb_responsive_options()->get_any_value( $args['props'], "{$background_prefix}repeat{$suffix}", $background_repeat_default, ! $is_prev_background_image_active );
					...
					// Blend.
					$background_blend_default = ET_Builder_Element::$_->array_get( $args['fields'], "{$background_prefix}blend.default", '' );
					$background_blend         = et_pb_responsive_options()->get_any_value( $args['props'], "{$background_prefix}blend{$suffix}", $background_blend_default, ! $is_prev_background_image_active );
					$background_blend_inherit = et_pb_responsive_options()->get_any_value( $args['props'], "{$background_prefix}blend{$suffix}", '', true );
				...
				if ( ! $has_background_gradient_and_image ) {
					...
					$background_color_enable  = ET_Builder_Element::$_->array_get( $args['props'], "{$background_prefix}enable_color{$suffix}", '' );
					$background_color_initial = 'off' === $background_color_enable && ! $is_desktop ? 'initial' : '';

					$background_color       = et_pb_responsive_options()->get_inheritance_background_value( $args['props'], "{$background_prefix}color", $device, $background_base_name, $args['fields'] );
				...
				if ( '' !== $background_style ) {
					$background_style_attrs = array(
						...
						'priority'    => $args['module']->_style_priority,
					);
					...
					ET_Builder_Element::set_style( $args['render_slug'], $background_style_attrs );
				}
			}

			// Background Hover.
			if ( et_builder_is_hover_enabled( "{$basename}_color", $args['props'] ) ) {
				...
				$background_color_gradient_overlays_image_desktop = et_pb_responsive_options()->get_any_value( $args['props'], "{$background_prefix}color_gradient_overlays_image", 'off', true );

				$gradient_properties_desktop = array(
					'type'             => et_pb_responsive_options()->get_any_value( $args['props'], "{$background_prefix}color_gradient_type", '', true ),
					'direction'        => et_pb_responsive_options()->get_any_value( $args['props'], "{$background_prefix}color_gradient_direction", '', true ),
					'radial_direction' => et_pb_responsive_options()->get_any_value( $args['props'], "{$background_prefix}color_gradient_direction_radial", '', true ),
					'color_start'      => et_pb_responsive_options()->get_any_value( $args['props'], "{$background_prefix}color_gradient_start", '', true ),
					'color_end'        => et_pb_responsive_options()->get_any_value( $args['props'], "{$background_prefix}color_gradient_end", '', true ),
					'start_position'   => et_pb_responsive_options()->get_any_value( $args['props'], "{$background_prefix}color_gradient_start_position", '', true ),
					'end_position'     => et_pb_responsive_options()->get_any_value( $args['props'], "{$background_prefix}color_gradient_end_position", '', true ),
				);
				...
				$use_background_color_gradient_hover = et_pb_responsive_options()->get_inheritance_background_value( $args['props'], "{$background_prefix}use_color_gradient", 'hover', $background_base_name, $args['fields'] );
				...
				if ( 'on' === $use_background_color_gradient_hover ) {
					...
					// Hover value.
					$background_color_gradient_type_hover             = et_pb_hover_options()->get_raw_value( "{$background_prefix}color_gradient_type", $args['props'], $background_color_gradient_type_desktop );
					$background_color_gradient_direction_hover        = et_pb_hover_options()->get_raw_value( "{$background_prefix}color_gradient_direction", $args['props'], $background_color_gradient_direction_desktop );
					$background_color_gradient_direction_radial_hover = et_pb_hover_options()->get_raw_value( "{$background_prefix}color_gradient_direction_radial", $args['props'], $background_color_gradient_radial_direction_desktop );
					$background_color_gradient_start_hover            = et_pb_hover_options()->get_raw_value( "{$background_prefix}color_gradient_start", $args['props'], $background_color_gradient_color_start_desktop );
					$background_color_gradient_end_hover              = et_pb_hover_options()->get_raw_value( "{$background_prefix}color_gradient_end", $args['props'], $background_color_gradient_color_end_desktop );
					$background_color_gradient_start_position_hover   = et_pb_hover_options()->get_raw_value( "{$background_prefix}color_gradient_start_position", $args['props'], $background_color_gradient_start_position_desktop );
					$background_color_gradient_end_position_hover     = et_pb_hover_options()->get_raw_value( "{$background_prefix}color_gradient_end_position", $args['props'], $background_color_gradient_end_position_desktop );
					$background_color_gradient_overlays_image_hover   = et_pb_hover_options()->get_raw_value( "{$background_prefix}color_gradient_overlays_image", $args['props'], $background_color_gradient_overlays_image_desktop );
					...
					$background_images_hover[] = $args['module']->get_gradient( $gradient_values_hover );
				...
				// fields. So, built in function get_value() doesn't work in this case.
				// Temporarily, we need to fetch the the value from get_raw_value().
				$background_image_hover = et_pb_responsive_options()->get_inheritance_background_value( $args['props'], "{$background_prefix}image", 'hover', $background_base_name, $args['fields'] );
				$parallax_hover         = et_pb_hover_options()->get_raw_value( "{$background_prefix}parallax", $args['props'] );
				...
				if ( '' !== $background_image_hover && null !== $background_image_hover && 'on' !== $parallax_hover ) {
					...
					// Size.
					$background_size_hover   = et_pb_hover_options()->get_raw_value( "{$background_prefix}size", $args['props'] );
					$background_size_desktop = ET_Builder_Element::$_->array_get( $args['props'], "{$background_prefix}size", '' );
					...
					// Position.
					$background_position_hover   = et_pb_hover_options()->get_raw_value( "{$background_prefix}position", $args['props'] );
					$background_position_desktop = ET_Builder_Element::$_->array_get( $args['props'], "{$background_prefix}position", '' );
					...
					// Repeat.
					$background_repeat_hover   = et_pb_hover_options()->get_raw_value( "{$background_prefix}repeat", $args['props'] );
					$background_repeat_desktop = ET_Builder_Element::$_->array_get( $args['props'], "{$background_prefix}repeat", '' );
					...
					// Blend.
					$background_blend_hover   = et_pb_hover_options()->get_raw_value( "{$background_prefix}blend", $args['props'] );
					$background_blend_default = ET_Builder_Element::$_->array_get( $args['fields'], "{$background_prefix}blend.default", '' );
					$background_blend_desktop = ET_Builder_Element::$_->array_get( $args['props'], "{$background_prefix}blend", '' );
				...
				if ( ! $has_background_gradient_and_image_hover ) {
					$background_color_hover = et_pb_responsive_options()->get_inheritance_background_value( $args['props'], "{$background_prefix}color", 'hover', $background_base_name, $args['fields'] );
				...
				if ( '' !== $background_hover_style ) {
					...
					ET_Builder_Element::set_style( $args['render_slug'], $background_hover_style_attrs );
				}
			}
		}
	}
